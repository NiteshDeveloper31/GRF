...
async function seed30Products() {
  try {
    ...
    // Load available local images and convert them to Base64
    const base64Images = [];
    ...
    IMAGE_PATHS.forEach(imgPath => {
      if (fs.existsSync(imgPath)) {
        ...
        console.log(`Loaded and base64-encoded image: ${path.basename(imgPath)}`);
      } else {
        console.warn(`Warning: Image not found at ${imgPath}`);
      }
    });

    // Fallback image if none could be loaded
    const fallbackImage = {
    ...
    // Seed 30 new products
    const seedList = PRODUCTS_DATA.map((prod, idx) => {
      // Rotate through loaded base64 images
      ...
      return {
        ...
        isActive: true,
        isFeatured: idx < 6, // Make first 6 products featured
        order: idx
      };
    });

    console.log(`Inserting ${seedList.length} products with actual industrial images...`);
